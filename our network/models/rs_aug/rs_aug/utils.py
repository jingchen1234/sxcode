...
def read_img(img_path):
    ...
    if img_format == 'tiff' or ext == '.img':
        ...
        if dataset == None:
            raise Exception('Can not open {}.'.format(img_path))
        ...
        if npy_data.shape[0] < npy_data.shape[1] and npy_data.shape[0] < npy_data.shape[2]:
...
def normalize(img, min_value, max_value, mean, std, band_num):
    ...
    img[:, :, :band_num] /= std
    return img
# def normalize(img, band_num):
#     range_value = [np.max(np.max(img[:, :, i])) - np.min(np.min(img[:, :, i])) for i in range(band_num)]
#     img[:, :, :band_num] = (img[:, :, :band_num].astype(np.float32) - \
#                            [np.min(np.min(img[:, :, i])) for i in range(band_num)]) / range_value
#     return img
...
def band_comput(img, b1, b2):
    ...
    out = (img[:, :, b1] - img[:, :, b2]) / (img[:, :, b1] + img[:, :, b2]+1e-8)
...
def pca(img, kp_num):
    ...
    img = img - np.mean(img, axis=0)  # 对原始数据进行中心化处理
    sigma = img.T.dot(img) / (len(img) - 1)  # 计算协方差矩阵
    a, w = np.linalg.eig(sigma)  # 计算协方差矩阵的特征值和特征向量
    sorted_indx = np.argsort(-a)  # 将特征向量按照特征值进行排序
    img = img.dot(w[:, sorted_indx[:kp_num]])  # 对数据进行降维
    # img_new = img_new.reshape((h, w, kp_num))
    return img
